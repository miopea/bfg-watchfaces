...
object Bake {
    ...
    fun main(args: Array<String>) {
        ...
        for (a in args) {
            ...
            val i = body.indexOf('=')
            if (i > 0) kv[body.substring(0, i)] = body.substring(i + 1) else kv[body] = "true"
        }
        ...
        println("  engine=${merged.engine} scale=${merged.scale} dial=${merged.dialColor} ink=${merged.inkColor}")

        // Refuse to write an artefact that would install and then never appear.
        val xml = com.bfg.watchfaces.generator.WffEmitter.emit(merged)
        ...
        Workbench.exportTo(root, merged, colors).forEach { println("  wrote $it") }
