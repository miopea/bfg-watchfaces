...
### First, nothing was installed

The phone was on 1.54 and the tilt shipped in 1.57. The relief is emitted by the
PHONE when it builds the face XML, so the build that emits it has to be the one
installed. Checked before theorising, which is the only reason the second cause
was ever reached.

### Then, the real bug

After 1.57 landed, the face was re-saved and sent — and the APK pulled back off
the watch still said:

```text
Generated by the BFG Watch Faces generator, v12
clock_relief / <Gyro>  →  0
```

`FaceLibrary.save` stamps the current `generatorVersion`, and it stamped the
copy it WROTE. The Studio screen went on holding the params it already had, so
**Save then Send built the face at the version it had before saving.** The stamp
was real, on disk, and invisible to the thing that used it.

Nobody would find this by reading either half: the store does exactly what it
documents, and the screen saves exactly what it holds. It only appears where
they meet, and only if you check the artefact rather than the intent.

`save` already returned the stored face; the screen now takes its params back
from that return instead of assuming it wrote what it sent. **The store is the
authority on what a saved face is.**

### The shape of the diagnosis

The version was read off the APK ON THE WATCH, not inferred from the phone
saying "Saved". Every step in this chain reported success — the save, the send,
the watch's `OK` — and the artefact at the end of it was still v12.

That is the third time in this project that a green path has ended in a wrong
artefact, and each time the same move found it: **open the thing that shipped**.
