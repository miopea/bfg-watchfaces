...
export async function reset(): Promise<void> {
  await env.DB.prepare("DELETE FROM automatic_publications").run();
  await env.DB.prepare("UPDATE automatic_approval_policy SET mode = 'recommendations', max_per_hour = 5, max_per_author_day = 1 WHERE id = 1").run();
